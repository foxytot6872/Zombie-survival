...
class BruteZombie(Enemy):
    ...
    def draw_body(self, surface: pygame.Surface):
        """Draw brute zombie body using sprite sheet"""
        ...
        if 0 <= self.frame_index < len(self.animation_frames):
            ...
            # Scale frame if brute zombie (bigger sprite)
            if hasattr(self, 'sprite_scale') and self.sprite_scale != 1.0:
                scaled_width = int(current_frame.get_width() * self.sprite_scale)
                scaled_height = int(current_frame.get_height() * self.sprite_scale)
